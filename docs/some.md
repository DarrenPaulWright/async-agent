...
## some(array, callback, [settings]) ⇒ <code>Promise</code>
> Determine whether at least one element in an array passes an async test.

**Returns**: <code>Promise</code> - The promise is resolved with true after one callback returns a truthy value or false after all callbacks return a falsey value  

| Param | Type | Default | Description |
| --- | --- | --- | --- |
| array | <code>Array</code> |  | An array to iterate over. |
| callback | <code>function</code> |  | __Parameters:__ element, index, and array.<br> __Context:__ same as that provided to the main function.<br> __Return:__ A truthy value to cancel further iterations.<br> _May return a Promise. Rejections are not caught._ |
