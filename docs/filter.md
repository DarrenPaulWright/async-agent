...
## filter(array, callback, [settings]) ⇒ <code>Promise</code>
> Create a new array with all the elements of an array that pass an async test.

**Returns**: <code>Promise</code> - The promise is resolved with a new array of all the values that passed the test.  

| Param | Type | Default | Description |
| --- | --- | --- | --- |
| array | <code>Array</code> |  | An array to iterate over. |
| callback | <code>function</code> |  | __Parameters:__ element, index, and array.<br>__Context:__ same as that provided to the main function.<br>__Return:__ A truthy value to include this element in the final result.<br>_May return a Promise. Rejections are not caught and any work done up to that point will be lost_. |
