...
<br><a name="includes"></a>

## includes(array, value, [settings]) ⇒ <code>Promise</code>
> Determines if an element is in an array. Uses SameValue comparison performed async.

**Returns**: <code>Promise</code> - The promise is resolved with true if a match is found or false if no matches are found.  
